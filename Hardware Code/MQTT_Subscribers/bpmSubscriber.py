...
MQTT_TOPIC = 'patient/'+ CIN +'/bpm'
CIN='13017750'
Patienturl= "http://192.168.4.175:8081/api/Patient/"+CIN

PowerBIurl=requests.get(Patienturl).json()['powerBILinkBpm']
print(PowerBIurl)
...
def on_message(client, userdata, msg):
    ...
    state="normal"
    if (int(v)<60 and int(v)>100):
        state="abnormal heart rate!"
    ...
    body= '{"patient": {"cin":"'+ CIN +'" }, "bpm": "' + v +'","gatewayID":"'+ MQTT_USER +'","healthState":"'+ state +'"}'
    print(body)
    body2='{"bpm":'+ v +',"txt":"bpmVal","date":'+ str(x) +'}'
