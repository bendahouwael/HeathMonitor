...
MQTT_TOPIC = 'patient/'+ CIN +'/respiratory'
CIN='13017750'
Patienturl= "http://192.168.4.175:8081/api/Patient/"+CIN

PowerBIurl=requests.get(Patienturl).json()['powerBILinkResp']
print(PowerBIurl)
...
def on_message(client, userdata, msg):
    ...
    state="normal"
    if (int(v)<12 and int(v)>25):
        state="abnormal respiratory rate!"
    ...
    body= '{"patient": {"cin":"'+ CIN +'" }, "respiratoryRate": "' + v +'","gatewayID":"'+ MQTT_USER +'","healthState":"'+ state +'"}'
    print(body)
    body2='{"respiratoryRate":'+ v +',"txt":"respVal","date":'+ str(x) +'}'
